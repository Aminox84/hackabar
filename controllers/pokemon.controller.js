...
module.exports.getPokemon = async (req, res) => {
    ...
    if(!ObjectId.isValid(id)){res.status(400);
        res.send(`Bad request ${id} is not a valid ID`)
        return;
    }

    const pokemon = await Pokemon.findOne({ _id: id}, function(err, pokemon) {
        if (err){
            res.status(400);
            res.send(`Bad request pokemon with id : ${id} don't exist`)
            return;
...
//         _id: ObjectId(),
//         "id": temp.pokemons[index].id,
//         price: 100,
//         "name": {
//           "english": temp.pokemons[index].name.english,
//           "japanese": temp.pokemons[index].name.japanese,
//           "french": temp.pokemons[index].name.french
//         },
//         "type": temp.pokemons[index].type,
//         "base": temp.pokemons[index].base
//       });
